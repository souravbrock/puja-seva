...
// URL paste + file upload (server converts uploads to WebP near-lossless).
function ImageField({ value, onChange, bucket }: { value: string; onChange: (url: string) => void; bucket?: string }) {
  ...
  const pick = async (f: File | undefined) => {
    ...
    setBusy(true);
    try {
      onChange(await uploadPhoto(f, bucket || 'images'));
      showToast('Photo uploaded (WebP)');
    } catch (e: unknown) {
  ...
  return (
    <div className="space-y-2">
      ...
      <label className={`inline-flex items-center gap-2 text-[13px] font-bold rounded-xl px-4 py-2.5 border cursor-pointer ${busy ? 'bg-stone-100 text-stone-400 border-stone-200' : 'bg-saffron-50 text-saffron-800 border-saffron-200'}`}>
        ...
        <input type="file" accept="image/*" className="hidden" disabled={busy} onChange={(e) => { void pick(e.target.files?.[0]); e.target.value = ''; }} />
      </label>
    </div>
  );
}
    ...
    <div className="space-y-4">
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={`${editRow ? 'Edit' : 'Add'} ${modal === 'lists' ? 'Samagri List' : modal}`} wide>
        <div className="space-y-3.5">
          ...
          {modal === 'pujas' && (<><div className="grid sm:grid-cols-2 gap-3.5"><Field label="Puja name"><Input value={form.name || ''} onChange={(e) => F('name', e.target.value)} /></Field><Field label="Category"><Input value={form.category || ''} onChange={(e) => F('category', e.target.value)} placeholder="Sanskar / Vrat / Havan" /></Field><Field label="Duration"><Input value={form.duration || ''} onChange={(e) => F('duration', e.target.value)} placeholder="e.g. 2-3 hours" /></Field><Field label="Base price"><Input type="number" min={0} value={form.base_price ?? ''} onChange={(e) => F('base_price', e.target.value)} /></Field></div><Field label="Description"><TextArea value={form.description || ''} onChange={(e) => F('description', e.target.value)} /></Field><Field label="Built-in art"><Select value={IMAGES.includes(form.image_url) ? form.image_url : ''} onChange={(e) => F('image_url', e.target.value)}><option value="">None — custom below</option>{IMAGES.map((u) => <option key={u} value={u}>{u}</option>)}</Select></Field><Field label="Custom image (paste URL or upload — stored as WebP)"><ImageField value={IMAGES.includes(form.image_url) ? '' : form.image_url || ''} onChange={(v) => F('image_url', v)} /></Field><Field label="Visible to customers"><Select value={String(form.is_active ?? true)} onChange={(e) => F('is_active', e.target.value === 'true')}><option value="true">Yes</option><option value="false">No</option></Select></Field></>)}
